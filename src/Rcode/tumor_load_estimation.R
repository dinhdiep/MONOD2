...
HaplotypeMixing <- function(range, numSims=3, mixingHapFiles, simulation.directory, return.amf.values = F, markers.info) {
  ...
  for(f in range) {
    ...
    if (return.amf.values) {
      ...
      amf <- read.table(paste0(simulation.directory,"/simulation.amf.matrix"),T,row=1) # subset to only the matching markers regions
...
colon.markers.file.name<-"colon_cancer_markers.txt"
lung.markers.file.name<-"lung_cancer_markers.txt"
...
cct.mixing.results <- HaplotypeMixing(range=ct.range, numSims=num.sims, cct.mixingHapFiles, simulation.directory=simulation_CCT_dir, return.amf.values=F, colon.markers)
lct.mixing.results <- HaplotypeMixing(range=ct.range, numSims=num.sims, lct.mixingHapFiles, simulation.directory=simulation_LCT_dir, return.amf.values=F, lung.markers)
...
# Make heatmaps
pdf("heatmap.pdf", width = 12, height = 15)
HeatMap(cct.matrix)
HeatMap(lct.matrix)
dev.off()
...
# Make boxplots
pdf("mhl_boxplot.pdf", width = 6, height = 1.5)
...
pdf("estimated_proportions.pdf", width = 3.0, height = 2.5, useDingbats=FALSE)
...
save.image("Tumor_load_estimate.RData")
